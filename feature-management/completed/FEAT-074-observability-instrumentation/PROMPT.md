...
## Overview
Add structured logging fields, tracing spans, and metrics across ccmux-server, plus expand status reporting for operational visibility.
...
- Extend `ccmux_get_status` to expose:
  - `commit_seq`
  - `clients`
  - replay buffer range
...
- [ ] Extend `ccmux_get_status` output to include commit sequence, client details, replay buffer range, and WAL/checkpoint health.
...
- [ ] `ccmux_get_status` returns commit sequence, client count/details, replay buffer range, and WAL/checkpoint health.
