...
pub async fn list_all_panes(
    ...
    debug!("ListAllPanes request from {} (filter: {:?})", ctx.client_id, session_filter);

    let session_manager = ctx.session_manager.read().await;

    // FEAT-078: Get client's focus state
    let client_focus = ctx.registry.get_client_focus(ctx.client_id);
...
pub async fn read_pane(
    ...
    debug!("ReadPane {} request from {} (lines: {})", pane_id, ctx.client_id, lines);
...
pub async fn get_pane_status(ctx: &HandlerContext, pane_id: Uuid) -> HandlerResult {
    debug!("GetPaneStatus {} request from {}", pane_id, ctx.client_id);
...
pub async fn create_pane_with_options(
    ctx: &HandlerContext,
    session_filter: Option<String>,
    ...
    debug!(
        ...
        model = ?claude_model,
        preset = ?preset,
        "create_pane_with_options called"
    );
    info!(
        "CreatePaneWithOptions request from {} (session: {:?}, window: {:?}, direction: {:?})",
        ctx.client_id, session_filter, window_filter, direction
    );
    ...
    let inherited_cwd = if cwd.is_none() {
        session.get_window(window_id)
            ...
            .and_then(|active_id| session.get_window(window_id).unwrap().get_pane(active_id))
            .and_then(|p| p.cwd())
            .map(String::from)
    } else {
        None
    ...
    if select {
        ...
        ctx.registry.update_client_focus(ctx.client_id, Some(session_id), Some(window_id), Some(pane_id));
    ...
    if let Some(wid) = select_window_id {
        let mut sm = ctx.session_manager.write().await;
...
pub async fn split_pane(
    ...
    info!(
        ...
        ctx.client_id, pane_id, direction, ratio
    );
    ...
    if select {
        ...
        // Also update client focus (FEAT-078)
        ctx.registry.update_client_focus(ctx.client_id, Some(session_id), Some(window_id), Some(new_pane_id));
    }
...
pub async fn resize_pane_delta(
    ctx: &HandlerContext,
    pane_id: Uuid,
    delta: f32,
) -> HandlerResult {
