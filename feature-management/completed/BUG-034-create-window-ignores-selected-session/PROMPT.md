# BUG-034: fugue_create_window Ignores Selected Session

## Summary
`fugue_create_window` creates windows in the wrong session. After calling `fugue_select_session` to switch to a different session, `fugue_create_window` (without explicit session parameter) still creates the window in the original session instead of the selected one.

## Steps to Reproduce

1. Start in session-0 (original session)
2. Create a new session "dev-qa" via `fugue_create_session`
3. Call `fugue_select_session` with dev-qa's session_id
4. Verify selection succeeded (returns `status: "selected"`)
5. Call `fugue_create_window` with name "logs" (no session parameter)
...
- `fugue_list_windows` for dev-qa shows only 1 window (the original)
- `fugue_list_windows` for session-0 shows 2 windows including "logs"

## Evidence from QA Run

```
# Selected dev-qa
fugue_select_session(session_id: "58816ea5-79a5-4c6c-a611-e800cddb4773")
-> {session_id: "58816ea5-...", status: "selected"}

# Created window without session param
fugue_create_window(name: "logs")
...
- **Workaround**: Always explicitly pass `session` parameter to `fugue_create_window`
