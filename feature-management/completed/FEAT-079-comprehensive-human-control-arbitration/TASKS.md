...
- [x] Read `ccmux-server/src/user_priority.rs` to understand current implementation.
...
- [x] Add `Arbitrator` checks to `ccmux_send_input` handler.
- [x] Add `Arbitrator` checks to `ccmux_resize_pane` / `ccmux_split_pane`.
- [x] Add `Arbitrator` checks to `ccmux_kill_pane` / `ccmux_kill_session`.
