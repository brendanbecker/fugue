...
- **Arbitration Logic**: Centralize logic in `fugue-server` to check permissions for *every* intent based on the source (Human vs MCP).
...
- [ ] Add `Arbitrator` checks to `fugue_send_input` handler.
- [ ] Add `Arbitrator` checks to `fugue_resize_pane` / `fugue_split_pane`.
- [ ] Add `Arbitrator` checks to `fugue_kill_pane` / `fugue_kill_session`.
