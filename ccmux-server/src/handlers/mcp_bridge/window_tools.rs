...
use uuid::Uuid;
use ccmux_protocol::{
    ErrorCode, ServerMessage, SplitDirection, WindowInfo,
};
...
pub async fn list_windows(
    ...
    debug!("ListWindows request from {} (filter: {:?})", ctx.client_id, session_filter);
    ...
    let session = if let Some(ref filter) = session_filter {
        ...
        ctx.resolve_active_session(&session_manager)
...
pub async fn create_window_with_options(
    ctx: &HandlerContext,
    session_filter: Option<String>,
    name: Option<String>,
    ...
    info!(
        ...
        ctx.client_id, session_filter, name, cwd
    );
